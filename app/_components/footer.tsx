import Link from "next/link";
import React from "react";
import img from "../../public/img.jpeg";
import Image from "next/image";
const Footer = () => {
  return (
    <>
      <div className="">
        <footer className="bg-white relative text-white bg-my_bg_image lg:py-16 bg-no-repeat bg-center">
          <div className="absolute top-0 bottom-0 left-0 right-0 bg-black/75 z-[2]" />
          <div className="mx-auto max-w-screen-2xl  pb-8 pt-16 sm:px-2 max-sm:px-4 relative z-10  ">
            {/* <div className="relative">
            <div className="bg-black-300 ">hello</div>
            <Image
              src={img}
              alt="img"
              className="absolute -z-10 w-full min-h-[350px] object-cover "
            />
          </div> */}
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-32 z-10 ">
              <div className="mt-2 max-w-md text-center leading-relaxed text-white sm:max-w-xs sm:text-left">
                <p className="text-text-white max-sm:text-left h3-bold ">
                  ROMMIECONNECTS
                </p>

                <p className="mt-4  text-text_white lg:text-left md:w-[500px] sm:w-full text-start p-regular-14">
                  Your No#1 Platform for finding real reviews. We believe in
                  real voices, genuine experiences, and the power of making
                  smart decisions. Benefit yourself by going deeper, discover
                  top-ranking products, and contribute your voice
                  to the community.
                </p>
                <div className="flex gap-3 mt-2  max-w-md text-center leading-relaxed text-white sm:max-w-xs sm:text-left">
                  <div className="flex gap-2">
                    <span>icon</span>
                    <span>info@example.com</span>
                  </div>
                  <div className="flex gap-2">
                    <span>icon</span>
                    <span>Livechat</span>
                  </div>
                </div>

                <ul className="mt-8 flex gap-6 sm:max-w-xs sm:text-center">
                  <div className="flex items-center justify-center bg-white rounded-[50%] h-7 w-7">
                    <li>
                      <a
                        href="#"
                        rel="noreferrer"
                        target="_blank"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        <span className="sr-only">Twitter</span>

                        <svg
                          className="h-6 w-6"
                          fill={"#FF6522"}
                          viewBox="0 0 24 24"
                          aria-hidden="true"
                        >
                          <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
                        </svg>
                      </a>
                    </li>
                  </div>
                  <div className="flex items-center justify-center bg-white rounded-[50%] h-7 w-7">
                    <li>
                      <a
                        href="#"
                        rel="noreferrer"
                        target="_blank"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        <span className="sr-only">Facebook</span>

                        <svg
                          className="h-6 w-6"
                          viewBox="0 0 24 24"
                          aria-hidden="true"
                          fill={"#FF6522"}
                        >
                          <path
                            fillRule="evenodd"
                            d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"
                            clipRule="evenodd"
                          />
                        </svg>
                      </a>
                    </li>
                  </div>
                  <div className="flex items-center justify-center bg-white rounded-[50%] h-7 w-7">
                    <li>
                      <a
                        href="#"
                        rel="noreferrer"
                        target="_blank"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        <span className="sr-only">Instagram</span>

                        <svg
                          className="h-6 w-6"
                          fill={"#FF6522"}
                          viewBox="0 0 24 24"
                          aria-hidden="true"
                        >
                          <path
                            fillRule="evenodd"
                            d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z"
                            clipRule="evenodd"
                          />
                        </svg>
                      </a>
                    </li>
                  </div>
                </ul>
              </div>
              <div className="grid grid-cols-1 gap-8  lg:grid-cols-3 lg:text-left">
                <div>
                  <p className=" text-white p-semibold-20">Home</p>

                  <ul className="mt-6 space-y-4 text-sm">
                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        Services
                      </a>
                    </li>

                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        About Us
                      </a>
                    </li>

                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        FAQ
                      </a>
                    </li>

                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        pricing
                      </a>
                    </li>

                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        Contact
                      </a>
                    </li>
                  </ul>
                </div>

                <div>
                  <p className=" text-white p-semibold-20">About</p>

                  <ul className="mt-6 space-y-4 text-sm">
                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        How it work
                      </a>
                    </li>

                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        Why Us
                      </a>
                    </li>

                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        Testimonials
                      </a>
                    </li>
                  </ul>
                </div>

                <div>
                  <p className=" text-white p-semibold-20">Services</p>

                  <ul className="mt-6 space-y-4 ">
                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        Room
                      </a>
                    </li>

                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        Car
                      </a>
                    </li>

                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        Restaurants
                      </a>
                    </li>
                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        Boat/Jet Skiing
                      </a>
                    </li>
                    <li>
                      <a
                        href="#"
                        className="text-white  transition hover:opacity-75 p-regular-18 "
                      >
                        Flight
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </footer>
      </div>

      <div className="max-w-screen-2xl m-auto py-4 max-sm:flex-col max-sm:px-4">
        <div className="text-center flex justify-between max-sm:flex-col">
          <div className=" p-semibold-18 text-white md:gap-10 flex justify-between max-sm:flex-col max-sm:text-left">
            <Link
              className="inline-block text-primary  transition hover:text-primary/75 "
              href="#"
            >
              Terms & Conditions
            </Link>

            <Link
              className="inline-block text-primary  transition hover:text-primary/75 "
              href="#"
            >
              Privacy Policy
            </Link>
          </div>
          <div className="p-semibold-18 text-white flex justify-between">
            <Link
              className="inline-block text-primary  transition hover:text-primary/75 "
              href="#"
            >
              Copyright Ⓒ Rommieconnnec. All Rights Reserved
            </Link>
          </div>
        </div>
      </div>
    </>
  );
};

export default Footer;
